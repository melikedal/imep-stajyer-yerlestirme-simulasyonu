...
4. Yerel Arama (Local Search) İyileştirmesi:
...
Yerleştirme sürecinin her adımını şeffaf bir şekilde takip etmenizi sağlar.Atama sürecindeki her adımın teknik gerekçesini (GNO, kontenjan, liyakat) anlık olarak raporlar. Algoritma seçimlerine göre çıktılar farklılık gösterir:
...
Metriklerin Analizi:
Memnuniyet Puanı: Öğrencilerin tercih sıralarına göre aldıkları puanların toplamıdır; Heuristik yöntem genellikle bu puanı daha yukarı taşır.

Çözüm Süresi: Algoritmanın hesaplamayı bitirme süresidir; Greedy yaklaşımı doğrudan atama yaptığı için daha hızlıdır.

İşlem ve Tur Sayısı: Sistemin karara ulaşmak için yaptığı iterasyon sayısıdır.
